...
import type { GetClusterHealthResponse, GetClusterLayoutResponse, GetClusterStatisticsResponse } from '@/types/garage';
...
export function ClusterOverview({ clusterId }: ClusterOverviewProps) {
    const healthQuery = useQuery<GetClusterHealthResponse>({
        ...
        queryFn: async () => {
            const res = await api.get<GetClusterHealthResponse>(proxyPath(clusterId, '/v2/GetClusterHealth'));
            return res.data;
        }
    });
    ...
    const statsQuery = useQuery<GetClusterStatisticsResponse>({
        ...
        queryFn: async () => {
            const res = await api.get<GetClusterStatisticsResponse>(proxyPath(clusterId, '/v2/GetClusterStatistics'));
            return res.data;
        }
    });
    ...
    const usableCapacity = layout?.roles?.reduce((sum, role) => sum + (role.usableCapacity ?? 0), 0) ?? 0;
    const capacityRatio = layout && totalCapacity > 0 ? Math.min(1, usableCapacity / totalCapacity) : 0;
    const assignedCapacityLabel = layout ? formatBytes(totalCapacity) : '-';
    const usableCapacityLabel = layout ? formatBytes(usableCapacity) : '-';

    const statusVariant = health?.status === 'healthy'
        ? 'success'
            ...
            {(healthQuery.error || layoutQuery.error || statsQuery.error) && (
                <Alert variant="destructive">
                    ...
                    <AlertDescription>
                        ...
                        {statsQuery.error && ` ${getApiErrorMessage(statsQuery.error, 'Failed to load statistics.')}`}
                    </AlertDescription>
                </Alert>
            )}
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3 text-sm">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="font-semibold text-slate-900">{layout?.stagedRoleChanges?.length ?? '-'}</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-muted-foreground">Partition size</span>
                            <span className="font-semibold text-slate-900">{layout ? formatBytes(layout.partitionSize) : '-'}</span>
                        </div>
                    </CardContent>
                </Card>
