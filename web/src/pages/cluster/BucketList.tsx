...
export function BucketList({ clusterId }: BucketListProps) {
    ...
    const { data: buckets = [], isLoading, error } = useQuery<ListBucketsResponseItem[]>({
        ...
        <div className="space-y-4">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                ...
                <Dialog open={isDialogOpen} onOpenChange={(open) => {
                    ...
                    <DialogContent>
                        ...
                        <DialogFooter>
                            <Button onClick={() => createMutation.mutate(newBucketName.trim())} disabled={!newBucketName.trim() || createMutation.isPending}>
            ...
            {error && (
                <Alert variant="destructive">
                    ...
                    <AlertDescription>{getApiErrorMessage(error, 'Buckets could not be loaded.')}</AlertDescription>
                </Alert>
            )}
                            ...
                            <TableRow key={bucket.id}>
                                ...
                                <TableCell>
                                    <div className="flex flex-wrap gap-1">
                                        {bucket.globalAliases.length > 0 ? bucket.globalAliases.map((alias) => (
                                ...
                                <TableCell>
                                    <div className="flex flex-wrap gap-1">
                                        {bucket.localAliases.length > 0 ? bucket.localAliases.map((alias) => (
                                ...
                                <TableCell className="text-xs text-muted-foreground">{formatDateTime(bucket.created)}</TableCell>
