...
import type { GetClusterLayoutResponse, GetClusterStatusResponse, LayoutNodeRole, NodeRoleChange, NodeResp } from '@/types/garage';
...
export function LayoutManager({ clusterId }: LayoutManagerProps) {
    ...
    const layoutQuery = useQuery<GetClusterLayoutResponse>({
        ...
        queryFn: async () => {
            const res = await api.get<GetClusterLayoutResponse>(proxyPath(clusterId, '/v2/GetClusterLayout'));
            return res.data;
        }
    });
    ...
    const statusQuery = useQuery<GetClusterStatusResponse>({
        ...
        queryFn: async () => {
            const res = await api.get<GetClusterStatusResponse>(proxyPath(clusterId, '/v2/GetClusterStatus'));
            return res.data;
        }
    });
    ...
    const updateLayoutMutation = useMutation({
        mutationFn: async (data: EditableNode) => {
            ...
            const capacityBytes = parsedCapacity === null ? null : Math.round(parsedCapacity * 1_000_000_000);
    ...
    if (layoutQuery.isLoading || statusQuery.isLoading) return <div className="p-4">Loading layout...</div>;
        ...
        return (
            <Alert variant="destructive">
                ...
                <AlertDescription>
                    ...
                    {statusQuery.error && ` ${getApiErrorMessage(statusQuery.error, 'Failed to load nodes.')}`}
                </AlertDescription>
            </Alert>
        );
    }
                                        ...
                                        <TableRow key={node.id}>
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex justify-end gap-2">
                                                    <Dialog open={isDialogOpen && selectedNode?.id === node.id} onOpenChange={(open) => {
                                                            ...
                                                            {selectedNode && (
                                                                <div className="grid gap-4 py-4">
                                                                    <div className="grid gap-2">
                                                                        <Label>Zone</Label>
                                                                        <Input value={selectedNode.zone} onChange={e => setSelectedNode({ ...selectedNode, zone: e.target.value })} />
                                                                    </div>
                                                                    <div className="grid gap-2">
                                                                        <Label>Capacity (GB)</Label>
                                                                        <Input type="number" value={selectedNode.capacity} onChange={e => setSelectedNode({ ...selectedNode, capacity: e.target.value })} placeholder="Leave empty for gateway" />
                                                                    </div>
                                                                    <div className="grid gap-2">
                                                                        <Label>Tags (comma separated)</Label>
                                                                        <Input value={selectedNode.tags} onChange={e => setSelectedNode({ ...selectedNode, tags: e.target.value })} />
                                                                    </div>
                                                                </div>
                                                            )}
                                                            <DialogFooter>
                                                                <Button onClick={() => selectedNode && updateLayoutMutation.mutate(selectedNode)} disabled={updateLayoutMutation.isPending}>
                                                    ...
                                                    {role && (
                                                        <Button
                                                            ...
                                                            onClick={() => {
                                                                if (confirm('Remove this node from the layout?')) removeNodeMutation.mutate(node.id);
                                                            }}
                                                        >
