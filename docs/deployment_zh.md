...
- **一体化（`garage-admin-all`）** — 单一镜像：Admin Console 内嵌 S3 Browser 远端，同源伺服于 `/s3-browser`（无代理跳转，无额外容器）。最简单的嵌入部署——单容器，仅需 Admin 密钥。嵌入式 FileBrowser 的数据仍通过 Admin BFF 流转，与组合部署一致。
